...
import {
  ...
  DEFAULT_MIN_TIMING_OBSERVATIONS, DEFAULT_SLOWNESS_RATIO_THRESHOLD, DEFAULT_STALL_RATE_THRESHOLD, DEFAULT_STALL_MULTIPLE, DEFAULT_MIN_MISS_COUNT,
  DEFAULT_WEAK_SPOT_SCORING_SETTINGS,
...
describe('shrink', () => {
  it('shrinks a low-n estimate toward 0 more than a high-n one, for the same raw value', () => {
    const lowN = shrink(1.0, DEFAULT_MIN_TIMING_OBSERVATIONS, DEFAULT_MIN_TIMING_OBSERVATIONS) // n=15 -> factor 15/30 = 0.5
    const highN = shrink(1.0, 150, DEFAULT_MIN_TIMING_OBSERVATIONS) // n=150 -> factor 150/165 ≈ 0.909
  ...
  it('approaches (never reaches) the raw value as n grows large', () => {
    const veryHighN = shrink(1.0, 100_000, DEFAULT_MIN_TIMING_OBSERVATIONS)
    expect(veryHighN).toBeGreaterThan(0.99)
    expect(veryHighN).toBeLessThan(1.0)
  })

  it('is 0 for a 0 raw value regardless of n', () => {
    expect(shrink(0, 1000, DEFAULT_MIN_TIMING_OBSERVATIONS)).toBe(0)
  })
})

describe('computeTokenTimingStats', () => {
  it('is undefined below the observation floor (n < 15)', () => {
    const intervals = Array(DEFAULT_MIN_TIMING_OBSERVATIONS - 1).fill(200)
    expect(computeTokenTimingStats(intervals, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)).toBeUndefined()
  })

  it('is defined exactly at the observation floor (n === 15)', () => {
    const intervals = Array(DEFAULT_MIN_TIMING_OBSERVATIONS).fill(200)
    const stats = computeTokenTimingStats(intervals, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
  ...
  it('computes the stall rate as the share of intervals exceeding 2x the scope median', () => {
    ...
    // exactly at the boundary is NOT counted, matches DEFAULT_STALL_MULTIPLE's
    // strict `>` comparison), 12 at 100ms.
    const intervals = [...Array(3).fill(500), ...Array(12).fill(100)]
    const stats = computeTokenTimingStats(intervals, 250, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
...
describe('evaluateTokenWeakness — miss signal', () => {
  it('is not weak below DEFAULT_MIN_MISS_COUNT with no timing data', () => {
    const v = evaluateTokenWeakness(DEFAULT_MIN_MISS_COUNT - 1, undefined, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.isWeak).toBe(false)
    expect(v.missWeak).toBe(false)
    expect(v.score).toBe(0)
  })

  it('is weak at exactly DEFAULT_MIN_MISS_COUNT', () => {
    const v = evaluateTokenWeakness(DEFAULT_MIN_MISS_COUNT, undefined, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
  ...
  it('a higher miss count produces a higher (but capped/log-scaled) score', () => {
    const low = evaluateTokenWeakness(2, undefined, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const high = evaluateTokenWeakness(50, undefined, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
...
describe('evaluateTokenWeakness — slowness signal, n-threshold boundary', () => {
  ...
  it('n=14 (below floor): never slow-weak regardless of how elevated the ratio would be', () => {
    ...
    const timing = computeTokenTimingStats(intervals, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(timing).toBeUndefined()
    const v = evaluateTokenWeakness(0, timing, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.isWeak).toBe(false)
    expect(v.slowWeak).toBe(false)
  })

  it('n=15 (at floor) with ratio >= 1.5: slow-weak', () => {
    const intervals = Array(15).fill(scopeMedian * DEFAULT_SLOWNESS_RATIO_THRESHOLD)
    const timing = computeTokenTimingStats(intervals, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const v = evaluateTokenWeakness(0, timing, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.isWeak).toBe(true)
  ...
  it('n=15 with ratio just under 1.5: not slow-weak', () => {
    const intervals = Array(15).fill(scopeMedian * (DEFAULT_SLOWNESS_RATIO_THRESHOLD - 0.01))
    const timing = computeTokenTimingStats(intervals, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const v = evaluateTokenWeakness(0, timing, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.slowWeak).toBe(false)
    expect(v.isWeak).toBe(false)
...
describe('evaluateTokenWeakness — stall signal, threshold boundary', () => {
  ...
  it('exactly at DEFAULT_STALL_RATE_THRESHOLD: stall-weak', () => {
    // 3/15 = 20% exactly.
    const intervals = [...Array(3).fill(scopeMedian * 3), ...Array(12).fill(scopeMedian)]
    const timing = computeTokenTimingStats(intervals, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(timing!.stallRate).toBeCloseTo(DEFAULT_STALL_RATE_THRESHOLD, 5)
    const v = evaluateTokenWeakness(0, timing, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.stallWeak).toBe(true)
    expect(v.isWeak).toBe(true)
  })

  it('just under DEFAULT_STALL_RATE_THRESHOLD: not stall-weak', () => {
    // 2/15 ≈ 13.3%
    const intervals = [...Array(2).fill(scopeMedian * 3), ...Array(13).fill(scopeMedian)]
    const timing = computeTokenTimingStats(intervals, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(timing!.stallRate).toBeLessThan(DEFAULT_STALL_RATE_THRESHOLD)
    const v = evaluateTokenWeakness(0, timing, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
...
describe('evaluateTokenWeakness — shrinkage: a single low-n outlier neither flags nor dominates', () => {
  ...
  it('a genuinely elevated low-n (15) timing signal produces a SMALLER score than the identical signal at high n (150)', () => {
    ...
    const lowN = computeTokenTimingStats(Array(15).fill(scopeMedian * 2.01), scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const highN = computeTokenTimingStats(Array(150).fill(scopeMedian * 2.01), scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const lowV = evaluateTokenWeakness(0, lowN, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const highV = evaluateTokenWeakness(0, highN, scopeMedian, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
...
describe('evaluateTokenWeakness — combined signals', () => {
  it('miss alone (no timing data at all — e.g. consent off) still activates weakness', () => {
    const v = evaluateTokenWeakness(DEFAULT_MIN_MISS_COUNT, undefined, 0, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.isWeak).toBe(true)
    expect(v.missWeak).toBe(true)
    expect(v.slowWeak).toBe(false)
    expect(v.stallWeak).toBe(false)
  ...
  it('neither miss nor timing crosses its threshold: not weak, score 0', () => {
    const timing = computeTokenTimingStats(Array(20).fill(200), 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS) // ratio 1.0, no stalls
    const v = evaluateTokenWeakness(1, timing, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    expect(v.isWeak).toBe(false)
    expect(v.score).toBe(0)
  })

  it('a token weak on BOTH miss and timing gets a higher score than either alone', () => {
    const timing = computeTokenTimingStats(Array(20).fill(200 * 2), 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const missOnly = evaluateTokenWeakness(10, undefined, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const timingOnly = evaluateTokenWeakness(0, timing, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
    const both = evaluateTokenWeakness(10, timing, 200, DEFAULT_WEAK_SPOT_SCORING_SETTINGS)
...
describe('DEFAULT_* pin — Weak Spot Settings modal defaults', () => {
  ...
  it('pins the DEFAULT_* constants the modal falls back to', () => {
    ...
    expect(DEFAULT_SLOWNESS_RATIO_THRESHOLD).toBe(1.5)
    expect(DEFAULT_STALL_MULTIPLE).toBe(2)
    expect(DEFAULT_STALL_RATE_THRESHOLD).toBe(0.2)
  })
})
