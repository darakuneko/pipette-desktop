...
describe('WordDisplay romaji mode', () => {
  it('colors kana characters as success up through kanaCompleted', () => {
    renderWord({ romajiGuide: { typed: 'de', remaining: 'xina-', kanaCompleted: 1, words: [], lineCount: 2, showRow: true } })
  ...
  it('shows every kana as muted before any keystroke is confirmed', () => {
    renderWord({ romajiGuide: { typed: '', remaining: 'dhina-', kanaCompleted: 0, words: [], lineCount: 2, showRow: true } })
    const word = screen.getByTestId('word-0')
    expect(word.querySelector('.text-success')).toBeNull()
  })

  it('colors the full word as success once every kana is confirmed', () => {
    renderWord({ word: 'あい', romajiGuide: { typed: 'ai', remaining: '', kanaCompleted: 2, words: [], lineCount: 2, showRow: true } })
    const word = screen.getByTestId('word-0')
    const successSpans = word.querySelectorAll('.text-success')
  ...
  it('renders no per-character error color even mid-word', () => {
    ...
    renderWord({ word: 'かんじ', romajiGuide: { typed: 'ka', remaining: 'nji', kanaCompleted: 1, words: [], lineCount: 2, showRow: true } })
    const word = screen.getByTestId('word-0')
  ...
  it('does not apply romaji coloring to a word that is not the current word', () => {
    renderWord({
      ...
      wordIndex: 1,
      currentWordIndex: 0,
      romajiGuide: { typed: 'a', remaining: '', kanaCompleted: 1, words: [], lineCount: 2, showRow: true },
    })
