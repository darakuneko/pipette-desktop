...
function isValidPrefs(value: unknown): value is PipetteSettings {
  ...
  if ('appliedKeymapLayout' in obj && obj.appliedKeymapLayout != null && typeof obj.appliedKeymapLayout !== 'string') return false
...
async function readData(uid: string): Promise<PipetteSettings | null> {
  try {
    ...
    return {
      ...
      appliedKeymapLayout: parsed.appliedKeymapLayout,
