...
async function captureRomajiInputScreenshot(page: Page): Promise<void> {
  ...
  // modal itself, explicitly pick the Romaji method on the unified 3-way
  // Direct/Romaji/Kana selector (Romaji is already the default for a
  // freshly-loaded kana-capable pack, but selecting it here keeps the
  // capture deterministic rather than relying on that default), then close
  // the modal before typing.
