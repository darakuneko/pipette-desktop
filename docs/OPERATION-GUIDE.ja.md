...
- レイヤーパネルは **View Matrix** パネルに置き換わり、**Done**（完了）トグル、選択中のキー用の **Row**（行）/ **Col**（列）セレクト、そして一番下に **Reset View Matrix**（ビューマトリクスをリセット）ボタンが表示されます。Reset をクリックして確認（**Reset?**）すると、カスタム位置がすべて削除され物理マトリクス順に戻ります
- キーをクリックすると選択され、キーマップ上でハイライト表示されます。**Row** / **Col** セレクトにはそのキーの実効位置（`0` からキーボードのマトリクスの行数/列数マイナス 1 まで）が即座に表示されます。どちらかのセレクトを変更すると即座に保存されます（保存ボタンは不要）。キーの物理位置と同じ値を選ぶと、カスタム位置は逆に削除されます
- Ctrl+クリック（macOS では Cmd+クリック）でキーを選択に追加・削除でき、Shift+クリックで範囲選択できます。選択中のキーはすべてハイライト表示されたままです。2 つ以上のキーを選択している間、**Row** / **Col** セレクトは空欄のプレースホルダーを表示します — 値を選ぶと、選択中の全キーにその行（または列）が一括で適用されます。各キーはもう一方の座標をそれぞれ保持します
- 2 つ以上のキーが同じ実効表示位置になっている場合、衝突が解消されるまでパネルに警告（例:「2 件のキーが同じ表示位置を共有しています」）が表示され続けます。編集自体はブロックされませんが、該当キー間の Auto Move の順序があいまいになります
...
- **Settings（設定）** — **Mode** 行（下記参照）、**Layer**（画面上のキーマップが使うベースレイヤー。レイヤーが 2 つ以上あるキーボードで表示）、**Lines（行）** / **Font**（読み取りウィンドウの行数とフォントサイズ — この 2 つは全モード共通）。MonkeyType の言語がアクティブな場合は、**MonkeyType** の項で説明する **Pattern（パターン）** / **Units（単位）** / **Option（オプション）** の行もここに表示されます
...
![タイピングテスト — モードモーダル（MonkeyType）](screenshots/typing-test-mode-monkeytype.png)

左側 Settings（設定）パネルの **Mode** 行に、現在のモード種別とソース（MonkeyType の言語、Tatoeba のパック、またはインポートしたテキスト）が表示されます。行をクリックすると Mode モーダルが開きます。4 つのタブから入力対象を選べます:
...
- 各行にはパック名と単語数が表示され、右から左に書く言語には **RTL** バッジも表示されます
...
**ひらがな** または **カタカナ** の言語パックを選択すると、Option 行に全幅の **Romaji（ローマ字入力）** ボタンも追加されます（words・time パターンのみ）。クリックすると判定を直接切り替えるのではなく **ローマ字入力の詳細設定** モーダルが開きます。モーダル内でローマ字入力を有効にすると、ボタン自体がアクセントカラーになります。

![タイピングテスト — ローマ字入力の詳細設定](screenshots/typing-test-romaji-settings.png)

モーダルには、ローマ字入力の有効/無効スイッチのほかに 3 つの設定があります。ガイド行のフォントサイズは常に共通の **Settings > Font** サイズに連動し、専用の設定項目はありません。
...
![タイピングテスト — モードモーダル（Tatoeba）](screenshots/typing-test-mode-tatoeba.png)

**Tatoeba** タブでダウンロード済みの言語パックを選択すると（未ダウンロードの場合は先にダウンロードしてください — 上記の **モード** を参照）、[Tatoeba Project](https://tatoeba.org) からサンプリングされた実際の文をタイプできます。1 回のテストにつき 5 文がサンプリングされます。
...
![タイピングテスト — モードモーダル（Aozora Bunko）](screenshots/typing-test-mode-aozora.png)
...
- **モード** の項で説明したデータセット更新バナーはここにも表示されます — 更新するとカタログの一覧自体が更新されます（取り込み済みのテキストには影響しません）

#### File Import（ファイル取込）

![タイピングテスト — モードモーダル（File Import）](screenshots/typing-test-mode-import.png)
...
- 各行にはテキスト名と単語数が表示されます。行をクリックすると選択、ゴミ箱アイコンをクリックすると削除されます
...
- **タスクトレイ常駐 (Stay in System Tray)**: ON の間はウィンドウを閉じても Pipette はタスクトレイに隠れて動作し続けます。トレイアイコンをクリックするとウィンドウが再表示され、トレイアイコンメニューの **Quit** で終了できます。トレイメニューには **Show** / **Quit** があります（現時点では英語表記のみ）。
