...
モーダルには、ローマ字入力の有効/無効スイッチのほかに 4 つの設定があります。

- **表示する文字列**: ガイド行のローマ字表記 — **ROMAJI**（大文字）/ **Romaji**（先頭大文字）/ **romaji**（小文字、既定）。表示のみで、受理される打鍵には一切影響しません
- **ガイドのフォントサイズ**: 既定では共通の **Settings > Font** サイズと連動します。連動をオフにすると、ガイド行だけにカスタムサイズを設定できます（読み取りウィンドウ本文のサイズは変わりません）
- **表示パターン**: 複数の綴りが定義されているかなについて、ガイド行がどの綴りスタイル（訓令式、C/Q表記、二重字、小文字x、小文字l）を優先して表示するかを選びます。複数のスタイルを同時に選択でき、たとえば「小文字x」と「訓令式」を両方選ぶと、それぞれのスタイルが該当するかなに個別に適用され、同じガイドの中に両方反映されます。先頭の **Base（ベース）** ボタンは選択をすべて解除し、ガイドを現在有効なベース（ヘボン式または訓令式）の綴りに戻します。**表示のみ** の設定で、他の入力パターンで入力しても正解のままです
- **入力パターン**: 2 段に分かれています。
  - **Base（ベース）**: **Hepburn（ヘボン式、shi/chi）** と **Kunrei（訓令式、si/ti）** — どちらか単独でもすべてのかなを綴れる 2 系統です。既定では両方有効で、両方 ON のままでも構いません。**最低 1 つは常に有効です** — 最後に残った 1 つのベースはボタンが無効化され、クリックしても解除できません
  - **Options（オプション）**: **C/Q**・**二重字**・**小文字x**・**小文字l** — 独立した代替綴りグループで、既定は全て有効です。オフにするとそのグループの綴りは入力として拒否されます。Base 行と異なり、有効なベースだけで全かなをカバーできるため、Options は全てオフにしても構いません
