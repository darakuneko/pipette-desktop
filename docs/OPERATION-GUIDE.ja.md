...
- Ctrl+クリック（macOS では Cmd+クリック）でキーを選択に追加・削除でき、Shift+クリックで範囲選択できます。選択中のキーはすべてハイライト表示されたままです。2 つ以上のキーを選択している間、**Row** / **Col** セレクトは空欄のプレースホルダーを表示します — 値を選ぶと、選択中の全キーにその行（または列）が一括で適用されます。各キーはもう一方の座標をそれぞれ保持します。この Ctrl+クリック / Shift+クリックのショートカットは、キーマップ下部の移動後のズームコントロール脇にヒントとして表示されます
