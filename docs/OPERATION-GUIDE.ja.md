...
**ひらがな** または **カタカナ** の言語パックを選択すると、Option 行に全幅の **Romaji（ローマ字）** ボタンも追加されます（words・time パターンのみ）。クリックすると判定を直接切り替えるのではなく **ローマ字詳細設定** モーダルが開きます。モーダル内でローマ字入力を有効にすると、ボタン自体がアクセントカラーになります。

![タイピングテスト — ローマ字詳細設定](screenshots/typing-test-romaji-settings.png)
...
- **表示パターン**: 複数の綴りが定義されているかなについて、ガイド行がどの綴りスタイル（訓令式、二重字、小文字x、小文字l など）を優先して表示するか、または **自動** にするかを選びます。**表示のみ** の設定で、実際にどの受理される綴りで入力しても正解のままです
- **入力パターン**: 綴りスタイル群ごとのトグルで、既定は全て有効です。あるスタイルをオフにすると、そのスタイル群の綴りは入力として拒否されます。**各かなの標準（正規）綴りは常に受理される**ため、トグルの組合せに関わらずどの単語も最後まで入力できます
