...
- **Settings（設定）** — **データ元** 行（下記参照）、**Layer**（画面上のキーマップが使うベースレイヤー。レイヤーが 2 つ以上あるキーボードで表示）、**Lines（行）** / **Font**（読み取りウィンドウの行数とフォントサイズ — この 2 つは全モード共通）。MonkeyType の言語がアクティブな場合は、**MonkeyType** の項で説明する **Pattern（パターン）** / **Units（単位）** / **Option（オプション）** の行もここに表示されます
...
**Tatoeba** タブでダウンロード済みの言語パックを選択すると（未ダウンロードの場合は先にダウンロードしてください — 上記の **データ元** を参照）、[Tatoeba Project](https://tatoeba.org) からサンプリングされた実際の文をタイプできます。1 回のテストにつき 5 文がサンプリングされます。
