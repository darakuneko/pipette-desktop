...
ローマ字入力は MonkeyType タブに限りません。ローマ字入力に対応するソース — **ひらがな** または **カタカナ** の MonkeyType 言語パック（words・time パターン）、かなの **Tatoeba** パック、かな純度100%の **File Import** / **Aozora Bunko** テキストのいずれか — が読み込まれていれば、Option 行に全幅の **Romaji（ローマ字入力）** ボタンが追加されます。対応する言語パックとインポート済みテキストは、一覧のどこに表示されていても **Romaji** バッジで示されます（上記の共有言語パック一覧、および下記の File Import / Aozora Bunko の項を参照）。インポートしたテキストの場合、対応可否は一覧に表示するたびにテキスト本体の内容から都度ローカルで判定されます — 保存も同期もされないため、内容とバッジがずれることはありません。Romaji ボタンをクリックすると判定を直接切り替えるのではなく **ローマ字入力の詳細設定** モーダルが開きます。モーダル内でローマ字入力を有効にすると、ボタン自体がアクセントカラーになります。
