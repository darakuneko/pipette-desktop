...
この設定はキーコードオーバーレイパネル (§3.13) でキーボードごとに切り替えでき、グローバルなデフォルトは設定 → デフォルト (§6.1) で設定できます。
...
**元に戻す (Undo)**: 選択中のキーに既にキーコードが割り当てられている場合、ポップオーバーに **Undo** ボタンが表示され、前のキーコードが表示されます。クリックすると前の割り当てに戻せます。
...
通常の文字キー、ファンクションキー、修飾キー、ナビゲーションキーなどの基本キーコードです。Basic タブは 4 つのビュータイプに対応しており、キーコードオーバーレイパネル (§3.13) から切り替えできます:
...
Combo タブは全エントリを一覧表示する**タイルグリッドプレビュー**と、Combo 関連のタイムアウト設定用の **QMK Settings** セクション（例: `COMBO_TERM`）を備えています。「これらの機能はキーボード全体に適用されます（現在のレイヤーだけではありません）」という注記が表示されます。
...
- 埋め込みの QMK Settings パネルで Combo タブから離れずにタイムアウト値を調整できます
...
### 4.1 2画面編集 (Split Edit)

キーボードレイアウトを 2 つ並べて表示し、レイヤー間のキー比較やコピーが可能です。

![2画面編集](screenshots/split-edit.png)

- ボタンをクリックして2画面編集を切り替えます
...
各キーコードタブから詳細設定モーダルを開けます。Lighting は設定ボタンから、Combo・Key Override・Alt Repeat Key はそれぞれのタブでタイルをクリックして開きます。
...
複数キーの同時押しで別のキーを発動させる設定です。Combo タブにインラインタイルグリッド一覧が表示され、タイルをクリックすると詳細エディタモーダルが開きます。

**タイルグリッド一覧 (Combo タブ)**

![Combo 一覧](screenshots/combo-modal.png)

Combo タブに番号付きタイルのグリッド (0--31) が表示されます。設定済みエントリはサマリー表示。タイルをクリックして詳細エディタへ。Combo キーコード (Combo On, Combo Off, Combo Toggle) がグリッドの下に表示されます。

**詳細エディタ**

![Combo 詳細](screenshots/combo-detail.png)

- 左パネル: Combo エディタ (Key 1--4, Output フィールド)。Combo タイムアウト設定は Combo タブの埋め込み QMK Settings で行います (§3.8)。
...
- **Back** ボタンでモーダル内のタイルグリッド一覧に戻る

### 5.3 Key Override

特定のキー入力を別のキーに置き換える設定です。Key Override タブにインラインタイルグリッド一覧が表示され、タイルをクリックすると詳細エディタモーダルが開きます。

**タイルグリッド一覧 (Key Override タブ)**

![Key Override 一覧](screenshots/key-override-modal.png)

番号付きタイルのグリッドが表示されます。設定済みエントリはサマリー表示。タイルをクリックして詳細エディタへ。
...
Repeat Key の代替動作を設定します。Alt Repeat Key タブにインラインタイルグリッド一覧が表示され、タイルをクリックすると詳細エディタモーダルが開きます。

**タイルグリッド一覧 (Alt Repeat Key タブ)**
...
キーコードタブバーの保存ボタン (フロッピーディスクアイコン)、またはキーコードオーバーレイパネルの Save タブ (§3.13) からエディタ設定パネルを開けます。
...
> **Note**: ツール設定 (キーボードレイアウト、Auto Advance、Key Tester、セキュリティ) はキーコードオーバーレイパネル (§3.13) に移動しました。ズームはツールバー (§4.2) で操作できます。レイヤー設定はエディタ左側のレイヤーパネルから直接管理します。
