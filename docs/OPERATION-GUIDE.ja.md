...
- **最後の状態で起動 (Restore Last Session)**: ON の間、Pipette は最後に接続していたキーボードを記憶し、次回起動時に自動で再接続します。表示していた画面はキーボードごとに記憶されているため、再接続によりそのキーボードで最後に使っていた画面も復元されます。起動から約 10 秒以内にキーボードが見つからない場合は静かに諦めます — 警告は表示されず、通常どおりデバイス選択画面のままになります。キーボードを手動で切断すると、記憶していたデバイスはクリアされます。
