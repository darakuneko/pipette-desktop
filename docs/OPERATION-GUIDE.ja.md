...
- **キーマップ履歴** — どの記録時点のキーマップを基準に分析するか選ぶ。**From** / **To** の編集は選んだスナップショットの有効期間内に閉じるため、スナップショットを必要とするタブ（ヒートマップ / Ergonomics / Layer activations）が複数のレイアウトを混ぜて表示することはない
- **From** / **To** — 分析対象の期間。両入力ともアクティブなスナップショット境界 (またはスナップショットが無いキーボードの場合は直近 7 日) に丸められる
- **デバイス** — `このデバイス` はこのマシンで記録したデータのみ、`全デバイス合算` は他マシンから同期したデータも含む。2 台選ぶと A / B 系列として並列表示される。間隔タブの表示を分布にしているときは非表示（分布ビンはデバイス別に分けないため）
