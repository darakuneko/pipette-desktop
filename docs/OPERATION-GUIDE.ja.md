...
- **アプリ / TypingTest** — セグメントトグルでこのフィルタ枠を 2 つのディメンションで切り替える。両者は排他で、同時に絞り込めるのはどちらか一方のみ（タイピングテストは必ず何らかのアプリの中で実行されるため）:
  - **アプリ** — マルチセレクトドロップダウン。期間中に観測されたアクティブアプリ名の一覧から任意組み合わせを選択。デフォルトは **全アプリ合算**（フィルタなし）。1 つ以上選ぶと、**アプリ別** タブを除く全チャートが選択アプリのいずれかでタグされた minute だけに絞り込まれる。Monitor App を有効にしてアプリ名がタグされた minute が記録されるまでドロップダウンは空。キーボード単位で永続化される
  ...
  - **アプリ別** タブでもトグルは表示されるが、このタブはアプリ横断で集計するため、アプリ側は「全アプリ合算」の意味になる（アプリのピッカーは出ない）。TypingTest 側は引き続きテストで絞り込める
