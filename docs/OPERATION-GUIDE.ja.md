...
**ひらがな** または **カタカナ** の言語パックを選択すると、Option 行に **Romaji（ローマ字）** トグルも追加されます（words・time パターンのみ）。ON にすると、原文との完全一致判定から、1 打鍵ずつのローマ字逐次判定に切り替わります。打鍵のたびに現在のかなに対して判定され、複数の綴りが定義されているかなはどれで打っても受理されます — 例えば「でぃ」は `dhi` / `deli` / `dexi` のどれで打っても OK です。
