...
- **表示パターン**: 複数の綴りが定義されているかなについて、ガイド行がどの綴りスタイル（訓令式、C/Q表記、二重字、小文字x、小文字l）を優先して表示するかを選びます。複数のスタイルを同時に選択でき、たとえば「小文字x」と「訓令式」を両方選ぶと、それぞれのスタイルが該当するかなに個別に適用され、同じガイドの中に両方反映されます。先頭の **Hepburn（ヘボン式）** ボタンは選択をすべて解除し、ガイドを既定のヘボン式ベースの綴りに戻します。**表示のみ** の設定で、他の入力パターンで入力しても正解のままです
- **入力パターン**: 綴りスタイル群ごとのトグルで、既定は全て有効です。あるスタイルをオフにすると、そのスタイル群の綴りは入力として拒否されます。**各かなのヘボン式綴りは常に受理される**（ヘボン式に定義がない音 — 小書き単体など — は IME 慣習の綴りがその役割を担います）ため、トグルの組合せに関わらずどの単語も最後まで入力できます
