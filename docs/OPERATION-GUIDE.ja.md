...
35 個すべてのカラートークンが必須です。インストール済みパックを export (行 → `.json`) すると完全なテンプレートが取得できます。
...
> **Note**: これはツールバーのキーボード単位ズーム (§4.1) とは別の機能です。§4.1 はキーマップエディタのみを拡大縮小しますが、ここの UI ズームはアプリケーションウィンドウ全体に適用されます。
...
6. アップロード完了後、エントリに **Open in Browser**、**Update on Hub**、**Remove from Hub** ボタンが表示される
