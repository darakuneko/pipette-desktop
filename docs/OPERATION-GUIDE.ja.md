...
- キーマップ操作はすべて無効になります: レイヤー切替、キー割り当て、キーポップオーバー、Key Tester（モード開始時に自動でオフ）
- レイヤーパネルは **View Matrix** パネルに置き換わり、**Reset View Matrix**（ビューマトリクスをリセット）ボタンが表示されます — クリックして確認（**Reset?**）すると、カスタム位置がすべて削除され物理マトリクス順に戻ります
- キーをクリックすると **Edit View Position**（表示位置を編集）ダイアログが開きます。**Matrix**（マトリクス）にキーの物理マトリクス位置が表示され（読み取り専用）、新しい **Row**（行）と **Col**（列）を入力して **Save**（保存）をクリックします。キーの物理位置と同じ値を入力すると、カスタム位置は逆に削除されます
- **View Matrix** 行の **Done**（完了）をクリックするとモードを終了します（キーボードの切替や切断時にも自動的に終了します）
