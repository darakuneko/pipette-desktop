...
**Mod Mask / Mod-Tap モード**

![キーポップオーバー — Modifier モード](screenshots/34-key-popover-modifier.png)

- **Mod Mask**: 修飾キーとキーを組み合わせます (例: `LSFT(KC_ESCAPE)`)
- **Mod-Tap**: 長押しで修飾キー、タップでキーを設定します
- 修飾キーチェックボックスで Left/Right の Ctrl、Shift、Alt、GUI を選択できます
- Left と Right の修飾キーは同時に選択できません — 片方を選択するともう片方は無効になります
