...
- **表示パターン**: 下の入力パターンと同じく 2 段に分かれています。
  ...
  - **Options（オプション）**: **C/Q**・**二重字**・**小文字x**・**小文字l** — 選択中の Base に重ねて適用する独立した代替綴りの優先設定で、既定は全てオフです。複数を同時に選択でき、たとえば「小文字x」と Kunrei ベースを両方選ぶと、それぞれのスタイルが該当するかなに個別に適用され、同じガイドの中に両方反映されます
