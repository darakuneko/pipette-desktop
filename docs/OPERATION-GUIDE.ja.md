...
| **活動量** | 時間 × 曜日グリッド or 年カレンダーを打鍵数 / WPM / セッションで色分け |
| **エルゴノミクス** | 指ごとの打鍵数をバーチャートで表示。指アサインは手動編集可。スナップショットが必要 |
| **バイグラム** | 連続する 2 打鍵 (バイグラム) の頻度・遅延ランキング、ペア別 / 指ペア別の打鍵間隔を表示 |
| **Layout Comparison** | 打鍵ログを別レイアウト（Colemak / Dvorak など）に乗せた場合の負担分布を試算。スナップショットが必要 |
| **レイヤー** | レイヤー別の打鍵数または layer-op 起動回数 |

ヒートマップ、エルゴノミクス、バイグラム > 指間隔ヒートマップ、Layout Comparison、レイヤー > 起動回数は期間に重なるキーマップスナップショットが必要。記録を有効にしている間に Pipette が自動でスナップショットを保存するため、空状態の案内に従って記録セッションを開始するとスナップショットが取れる。
...
- **グリッド** — 既存の 24 × 7（時間 × 曜日）グリッド（指標 = セッション のときはヒストグラムに切替）。既存の Period がグリッドの時間幅を決める
- **カレンダー** — GitHub 風の年間ヒートマップ。フィルタ行に年セレクタが出現し、選択した暦年を描画。現在年は今日でストップ（未来日は描画しない）
...
**カレンダー専用コントロール**（ビュー = カレンダー）

- **正規化** — `絶対値` はグリッド内のピーク日基準、`週内シェア` は所属する週の合計で割り、`年間シェア` は表示範囲全体の合計で割る
- **年** — キーマップ snapshot の保存年と現在年から候補を生成
...
- **しきい値で全件フィルタされた** — **平均間隔** の値が高すぎて生き残るペアが無くなった場合、指間隔とペア間隔の両クォドラントが「この期間のバイグラムデータはまだありません」にフォールバックする。しきい値を下げて表示を戻す
