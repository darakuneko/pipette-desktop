...
チャート上部のタブバーから 8 種類の分析を切り替えられる:

| タブ | 内容 |
|------|------|
| **ヒートマップ** | 物理キーごとの押下回数をキーマップ上に重ねて表示（レイヤー別）。期間内のキーマップスナップショットが必要 |
| **WPM** | Words-per-minute の推移、または時間帯別分布 |
| **間隔** | 打鍵間隔の分位数（min / p25 / 中央値 / p75 / max）を時系列または分布で表示 |
| **活動量** | 時間 × 曜日グリッドを打鍵数 / WPM / セッションで色分け |
| **エルゴノミクス** | 指ごとの打鍵数をバーチャートで表示。指アサインは手動編集可。スナップショットが必要 |
| **バイグラム** | 連続する 2 打鍵 (バイグラム) の頻度・遅延ランキング、ペア別 / 指ペア別の打鍵間隔を表示 |
| **Layout Optimizer** | 打鍵ログを別レイアウト（Colemak / Dvorak など）に乗せた場合の負担分布を試算。スナップショットが必要 |
| **レイヤー** | レイヤー別の打鍵数または layer-op 起動回数 |

ヒートマップ、エルゴノミクス、バイグラム > 指間隔ヒートマップ、Layout Optimizer、レイヤー > 起動回数は期間に重なるキーマップスナップショットが必要。記録を有効にしている間に Pipette が自動でスナップショットを保存するため、空状態の案内に従って記録セッションを開始するとスナップショットが取れる。

**共通フィルタ**

常に使えるフィルタ:

- **キーマップ履歴** — どの記録時点のキーマップを基準に分析するか選ぶ。**From** / **To** の編集は選んだスナップショットの有効期間内に閉じるため、スナップショットを必要とするタブ（ヒートマップ / Ergonomics / Bigrams 指間隔ヒートマップ / Layer activations）が複数のレイアウトを混ぜて表示することはない
- **From** / **To** — 分析対象の期間。両入力ともアクティブなスナップショット境界 (またはスナップショットが無いキーボードの場合は直近 7 日) に丸められる
- **デバイス** — `このデバイス` はこのマシンで記録したデータのみ、`全デバイス合算` は他マシンから同期したデータも含む。同時に選べるのは 1 つのみ。間隔タブの表示を分布にしているときは非表示（分布ビンはデバイス別に分けないため）
...
活動量タブは打鍵を「曜日 × 時間帯」で集計し、どの時間帯にどれだけ打鍵しているかを俯瞰する。

**指標**

- **打鍵数** — 24 × 7 のグリッドを打鍵数で色付けする。空セルは薄く、最多セルは最も濃い色に。0 でないセルにマウスを乗せると「月 09:00 — 1,234 打鍵 (総打鍵の 5.2%)」のように回数と期間全体に対する比率を併記する（空セルは `0 打鍵` のまま）

  ![分析 — 活動量 打鍵数](screenshots/analyze-activity-keystrokes.png)

- **WPM** — 同じグリッドをセル別平均 WPM で色付けする。**最小サンプル** を満たさないセルは色スケールに貼り付けず、薄色で表示される
- **セッション** — セッション長の 7 ビンヒストグラム（`5分未満`、`5-15分`、`15-30分`、`30-60分`、`1-2時間`、`2-4時間`、`4時間以上`）。セッションは「5 分以上の無入力で区切られた連続打鍵」
...
ビューは 2×2 のグリッドで、各クォドラントに件数セレクタ (10 / 20 / 30 / … / 100) が付く。デフォルトはランキング系が 10、バーチャート系が 20。

| クォドラント | 内容 |
|--------------|------|
| **頻出ペア** | バイグラムを出現回数で降順ランキング。**回数** または **平均間隔** 列をクリックして並び替え可能 |
| **遅いペア** | バイグラムを平均 IKI で降順ランキング (最も遅いペアが上)。**回数** / **平均間隔** / **p95 間隔** のいずれでも並び替え可能 |
| **指間隔ヒートマップ** | (送り指 → 受け指) ペアの平均 IKI バーチャート。左手始まりは青、右手始まりは赤で色分け |
| **ペアヒートマップ** | 個別キーペア (`A → B`、`LT1(KC_SPACE) → LShift` など) ごとの平均 IKI バーチャート |

![分析 — バイグラム](screenshots/analyze-bigrams.png)

**スナップショット要件**

キーマップスナップショットが必要なのは **指間隔ヒートマップ** クォドラントだけ。バイグラムペアの数値キーコードを指に解決するためにスナップショットの keymap + layout が要る。**頻出ペア** / **遅いペア** / **ペアヒートマップ** はペア集計データから直接描画されるためスナップショットなしでも動作する。
...
- **スナップショット無し (指間隔ヒートマップのみ)** — 「指ヒートマップには keymap snapshot が必要です。Record セッションを開始するか snapshot のある期間を選択してください」 他の 3 クォドラントは引き続き表示される

#### Layout Optimizer

Layout Optimizer は、記録した打鍵ログを別レイアウト（Colemak / Dvorak / Colemak DH ほか 30+ 種類）に乗せた場合の指 / 手 / 行の負担分布をシミュレートする。ファームウェアを書き換えずに「もし Colemak だったら」を試算できる。
