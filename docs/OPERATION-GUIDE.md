...
- Click a key to select it — it's highlighted on the keymap, and the **Row** / **Col** selects immediately show its effective position (`0` up to the keyboard's matrix row/column count minus 1). Changing either select saves instantly; there is no separate Save step. Choosing the value equal to the key's own physical position removes its custom position instead
- Ctrl-click (or Cmd-click on macOS) adds or removes a key from the selection; Shift-click selects a contiguous range. All selected keys stay highlighted. With 2 or more keys selected, the **Row** / **Col** selects show a blank placeholder — picking a value bulk-applies that row (or column) to every selected key in one step, each key keeping its own value on the other axis
- If two or more keys resolve to the same effective view position, the panel shows a persistent warning (e.g. "2 key(s) share the same view position") until the collision is resolved. Editing isn't blocked, but the Auto Move order between those keys becomes ambiguous
