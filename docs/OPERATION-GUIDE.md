...
**A successful Rewrite resets the Keyboard Layout dropdown back to QWERTY.** The keycap legends switch to the raw, untranslated keycode each key now actually sends, with no remap colouring — the same clean, undecorated state a snapshot / `.vil` restore leaves. Picking that same arrangement again afterward opens the confirmation dialog again like any other pick, since the dropdown no longer has any record of what was last rewritten.

**Display Only is the single remap-display mode, and it is always non-destructive.** Whenever a `keymapApplicable` (or any other) label set is the active **Keyboard Layout** selection, the Keymap Editor shows that pack's converted legends, with the affected keys tinted in one of two colours depending on what the tint is telling you. The key picker only follows suit for label sets that don't qualify as a clean, closed QWERTY permutation — the same eligibility check that decides whether a set can be Rewritten at all (JIS shift-pair legends, kana, and any partial/non-closed swap are common examples). A set that does qualify (Colemak, Dvorak, Eucalyn, …) only swaps which key sends a given character, and every one of those characters already appears somewhere in the picker, so the picker intentionally keeps its standard legends in every mode — only the Keymap Editor changes. The legends are always recomputed fresh from the keymap's actual, raw keycodes, so switching directly from one arrangement to another (e.g. Colemak straight to Dvorak) never stacks or double-converts — each pack's legend is derived independently from the same underlying keymap. The dropdown's selection is remembered per keyboard and restored automatically the next time that keyboard reconnects; only a successful Rewrite forces it back to QWERTY.

**The two remap tint colours.** A clean, closed QWERTY permutation pack (Colemak, Dvorak, Eucalyn, …) shown in Display Only mode is a *simulation* — the legend shows what a Rewrite would type, but pressing the key still sends the pre-Rewrite character — so its changed keys tint in the **simulated** colour (`key-label-simulated`). A pack that doesn't qualify as a permutation (JIS shift-pair legends, kana, …) shows *truthful* legends — the key really does produce what's shown — so it tints in the **actual** colour (`key-label-remap`) instead. A theme pack can define its own `key-label-simulated`; if it doesn't, Pipette derives one automatically from that pack's `key-label-remap` (see §6.4 below).

**QWERTY is always display-only.** Selecting QWERTY from the dropdown never opens the confirmation dialog and never touches the keymap — it only switches which legends are shown, back to raw and uncoloured. There is no "restore rewrite" offered by picking QWERTY; once a Rewrite has landed, only a previously saved `.vil` file or snapshot can bring back the keymap it replaced (see **Limitations** below).
