...
- **Guide spelling pattern**: which alternate spelling style(s) (kunrei, C/Q, digraph, small-x, small-l forms) the guide line prefers to show for kana with multiple accepted spellings. Multiple styles can be selected at once — e.g. selecting both Small x and Kunrei applies each preference to whichever kana it matches, in the same guide. The leading **Hepburn** button clears every selection and returns the guide to the canonical Hepburn-based spelling (the default). **Display only** — whichever accepted spelling you actually type is still correct, regardless of what the guide shows.
- **Accepted input patterns**: one toggle per alternate-spelling style family, all enabled by default. Turning a style off rejects that family's spellings as input; the **Hepburn spelling for every kana is always accepted** (for sounds Hepburn does not define, such as standalone small kana, the conventional IME spelling plays that role), so any combination of toggles still lets you type every word to completion.
