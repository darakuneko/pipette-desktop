...
- **Guide spelling pattern**: which alternate spelling style(s) (kunrei, C/Q, digraph, small-x, small-l forms) the guide line prefers to show for kana with multiple accepted spellings. Multiple styles can be selected at once — e.g. selecting both Small x and Kunrei applies each preference to whichever kana it matches, in the same guide. The leading **Base** button clears every selection and returns the guide to the canonical spelling of whichever base system (Hepburn or Kunrei) is currently enabled below. **Display only** — whichever accepted spelling you actually type is still correct, regardless of what the guide shows.
- **Accepted input patterns**: split into two rows.
  - **Base**: **Hepburn** (shi/chi) and **Kunrei** (si/ti), either of which can spell every kana on its own. Both are enabled by default and can be combined; **at least one must stay enabled** — the last remaining base toggle is disabled and ignores clicks.
  - **Options**: **C/Q**, **Digraph**, **Small x**, and **Small l** — independent alternate-spelling families, all enabled by default. Turning any of them off rejects that family's spellings as input; unlike the base row, every option can be turned off at once, since the enabled base(s) already cover every kana on their own.
